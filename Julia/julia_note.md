...
`key`如果摻雜`\r\n`、`\n`這種不可見字，會error。`key`也可能被打包成`vector`導致找不到。要初始化`add_agent!`
還有一對macro
***
3. 
`for`從1開始
***
4. macro超級噁心，不是單純的替換文字，是自製語法。乾淨衛生不汙染同時要手動。
***
5. `module`區域是不可見的(類似private)外面無法呼叫裡面除非`export`
***
6. 是`println`不是`printIn`，字體很重要